...
const REGEX_LITERAL_TERMINAL: &'static str = r"(?P<terminal>\s|\)|$)";

lazy_static! {
    pub static ref REGEX_WHITESPACE: Regex = Regex::new(r"^\s+").unwrap();

    // Identifier:
    pub static ref REGEX_IDENTIFIER: Regex = Regex::new(r"^([A-Za-z][A-Za-z0-9_]*):").unwrap();
    ...
    // Punctuation:
    pub static ref REGEX_PUNCTUATION_COLON: Regex = Regex::new(r"^:\S").unwrap();
    ...
    ^(?:
        (?:(?P<fterm>[^\s)]+)(?:~(?P<distance>\d*)))| # matches a term with fuzzy suffix (e.g. term1~, term2~3, ...), or
        (?P<term>[^\s)]+) # matches a term without fuzzy suffix (e.g. term1, term2)
    )
    {REGEX_LITERAL_TERMINAL}
    ")).unwrap();
}
